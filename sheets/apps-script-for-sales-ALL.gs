 ...
 *
 * 영문 카피(짧고 자연스러운 문장):
 * 채팅에서 Grok에게 「Titles 영문 채워줘」 요청 (권장)
 * 비상용 기계번역: LENA → 한글→영문 채우기(기계·빈 칸만)
 */
...
function buildLenaMenu_() {
  SpreadsheetApp.getUi()
    ...
    .addItem("한글→영문 기계번역 (빈 칸만·비상용)", "fillEmptyEnglishFromKorean")
    .addItem("한글→영문 기계번역 덮어쓰기 (선택 행)", "refillEnglishFromKoreanSelected")
    .addToUi();
}

// ─── 한글 → 영문 (빈 title/coverCopy/synopsis/authorBio) ───
// 상세 구현: apps-script-for-sales-translate-ko-en.gs 와 동일 함수명
// ALL.gs 단독 설치 시에도 동작하도록 아래에 포함
  ...
  try {
    ...
    for (var r = rowStart; r <= rowEnd; r++) {
      for (var p = 0; p < pairCols.length; p++) {
        ...
        try {
          ...
          // 기계번역만 (자연스러운 카피 → 채팅 Grok 권장)
          var translated = translateFieldKoToEn_(koVal, pair.kind, false);
          if (!translated) {
            skipped++;
            continue;
          }
    ...
    ui.alert(
      ...
      "채운 칸: " +
        ...
        "개\n" +
        (errors ? "오류: " + errors + "개\n" : "") +
        "엔진: LanguageApp (기계번역·비상용)\n\n" +
        (samples.length ? "예시:\n• " + samples.join("\n• ") + "\n\n" : "") +
        "※ 짧고 자연스러운 영문 카피는 채팅에서 Grok에게\n" +
...
function resolveKoEnColumns_(headers) {
  ...
  for (var i = 0; i < KO_EN_FIELD_PAIRS.length; i++) {
    ...
    var enCol = findHeaderColTranslate_(headers, pair.en);
    var koCol = findHeaderColTranslate_(headers, pair.ko);
    if (enCol && koCol) {
...
function findHeaderColTranslate_(headers, names) {
...
function translateFieldKoToEn_(koText, kind, preferGemini) {
  ...
  if (!plain) return "";
  // preferGemini 인자는 호환용(무시). 자연스러운 영문은 Grok 채팅 권장.
...
function getRunColorHex_(style) {
  ...
  try {
    ...
    if (obj && obj.getColorType) {
      // ColorType.RGB = 1 in Apps Script
